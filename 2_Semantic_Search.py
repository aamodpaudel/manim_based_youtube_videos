...
class SemanticMathAnimations(Scene):
    def construct(self):
        ...
        vec_b_math = MathTex(r"\vec{v}_B = [0.87, \ 0.48]", color=TEXT_COLOR).scale(0.9)
...
class SemanticTextSimilarity(Scene):
    def construct(self):
        ...
        header_2 = Text("Similarity Score", color=TITLE_COLOR, font_size=28, weight=NORMAL)
...
class SemanticAgentRouting(Scene):
    def construct(self):
        ...
        query_text = Text(
            '"Why did our cloud bill spike last month and how can we reduce it efficiently?"',
            color=PRIMARY_COLOR,
            font_size=30, 
            line_spacing=1.2
...
# --- Configuration & Palette ---
# Rule 1a: Primary Color
PRIMARY_COLOR = "#db2777"     # Pinkish-Red
# Rule 1d: Supplementary Palette
SECONDARY_BG = WHITE          # Chosen for Rule 1c compatibility
ACCENT_COLOR = "#be185d"      # Darker shade for contrast
TEXT_COLOR = BLACK            # For Title and labels
DIM_GRAY = "#6b7280"          # For supplementary info
GRID_COLOR = "#9ca3af"        # Slightly darker gray for visible arrows

class CloudVectorRepresentation(Scene):
    def construct(self):
        # Rule 1b: Background configuration
        self.camera.background_color = SECONDARY_BG
        ...
        # Adjusted X-coordinates to prevent overlap for long strings
        # Rule 2a & 2b: Strict spacing and no touching elements
        TXT_X = -5.8   # Column 1: Input Text (Left Aligned)
        ARR_X = -1.0   # Column 2: Arrows (Positioned between text and vector)
        VEC_X = 0.5    # Column 3: Vector Math (Left Aligned)

        # --- 3. ROW 1: PRIMARY PHRASE ---
        word_cloud = Text('"Reduce cloud costs"', color=PRIMARY_COLOR, font_size=28, weight=BOLD)
        word_cloud.move_to([TXT_X, 1.5, 0], aligned_edge=LEFT)
        
        arrow_1 = Arrow(LEFT, RIGHT, color=GRID_COLOR, buff=0.1).scale(0.6).move_to([ARR_X, 1.5, 0])
        ...
        # Row 1 Animation: Defining the baseline
        self.play(
            Write(word_cloud),
            GrowArrow(arrow_1),
            Write(vector_1)
        ...
        # Rule 2c: Legible labels and note
        note = Text("Coordinates represent semantic meaning", 
                    color=DIM_GRAY, font_size=16, slant=ITALIC)
        note.next_to(vector_1, UP, buff=0.2, aligned_edge=LEFT)
        self.play(FadeIn(note))
        self.wait(1)
        self.play(FadeOut(note))

        # Row 2 & 3: Comparison logic
        # Shows how context shifts the numerical representation
        self.play(
            Write(context_tech), 
            GrowArrow(arrow_tech), 
            Write(vector_tech),
            run_time=1.5
        )
        self.wait(0.5)
        ...
        self.play(
            Write(context_weather), 
            GrowArrow(arrow_weather), 
            Write(vector_weather),
            run_time=1.5
        )
        
        # Highlight: The logic of Vector Space
        summary_label = Text(
            "The same word can have different vector representation based on intent & context.", 
            color=TEXT_COLOR, font_size=18
        ).to_edge(DOWN, buff=1.0)
        
        # Creating a focus box around the vectors to highlight the difference
...
TITLE_COLOR = BLACK            # Rule 1c: Black & Not Bold
GRID_COLOR = "#e5e7eb"         # Light grey borders
LLM_FILL = "#fdf2f8"           # Light pink for LLM box

class FullAgentRoutingArchitecture(Scene):
    def construct(self):
        self.camera.background_color = SECONDARY_COLOR

        # --- 1. TITLE ---
        title = Text(
            "Multi-Agent Intent Routing & Ambiguity Resolution", 
            color=TITLE_COLOR, 
            font_size=22, 
            weight=NORMAL
        )
        ...
        # --- 2. INPUT SECTION ---
        input_box = RoundedRectangle(corner_radius=0.1, height=1.0, width=2.4, color=GRID_COLOR)
        input_label = Text("User Input", font_size=14, color=TEXT_COLOR).move_to(input_box.get_top() + DOWN*0.2)
        query_text = MarkupText(
            f'“How to reduce cloud cost,\n<span foreground="{PRIMARY_COLOR}">efficiently</span>”', 
            font_size=12, color=TEXT_COLOR, line_spacing=1.2
        ).next_to(input_label, DOWN, buff=0.1)
        input_grp = VGroup(input_box, input_label, query_text)

        # --- 3. ROUTER & LLM ---
        router_box = RoundedRectangle(corner_radius=0.1, height=1.4, width=2.6, color=TEXT_COLOR)
        router_label = Text("Semantic Router\n(Similarity)", font_size=14, color=TEXT_COLOR, line_spacing=1.2).move_to(router_box.get_center())
        router_grp = VGroup(router_box, router_label)

        llm_box = RoundedRectangle(corner_radius=0.1, height=1.0, width=2.6, color=PRIMARY_COLOR, fill_color=LLM_FILL, fill_opacity=1)
        llm_label = Text("Reasoning AI (LLM)", font_size=14, color=PRIMARY_COLOR, weight=BOLD).move_to(llm_box.get_center())
        llm_grp = VGroup(llm_box, llm_label)

        # --- 4. THE 5 AGENTS ---
        agents_data = [
            ["Agent 1: Finance", "0.78"],
            ["Agent 2: Cloud Cost", "0.96"],
            ["Agent 3: HR Assistant", "0.12"],
            ["Agent 4: DevOps", "0.965"],
            ["Agent 5: Sales Admin", "0.15"]
        ]

        agents_grp = VGroup()
        for i, (name, score) in enumerate(agents_data):
            # Agent 2 and 4 get highlight-ready colors
            is_contender = i in [1, 3]
            box = RoundedRectangle(corner_radius=0.05, height=0.5, width=3.2, color=GRID_COLOR)
            label = Text(name, color=TEXT_COLOR, font_size=12).move_to(box.get_left() + RIGHT*1.15)
            val = Text(score, color=PRIMARY_COLOR if is_contender else TEXT_COLOR, font_size=12).move_to(box.get_right() + LEFT*0.4)
            agents_grp.add(VGroup(box, label, val))
        
        agents_grp.arrange(DOWN, buff=0.15)

        # --- 5. LAYOUT & CENTERING ---
        architecture = VGroup(input_grp, router_grp, agents_grp).arrange(RIGHT, buff=1.0)
        llm_grp.next_to(router_grp, UP, buff=0.8)
        
        # Master group to center everything vertically
        content = VGroup(title, VGroup(architecture, llm_grp)).arrange(DOWN, buff=0.8)
        content.move_to(ORIGIN)

        # --- 6. CONNECTIONS ---
        arrow_in = Arrow(input_box.get_right(), router_box.get_left(), color=GRID_COLOR, buff=0.1)
        llm_comm = DoubleArrow(router_box.get_top(), llm_box.get_bottom(), color=PRIMARY_COLOR, buff=0.1, stroke_width=3)
        
        agent_paths = VGroup()
        for agent in agents_grp:
            agent_paths.add(Line(router_box.get_right(), agent.get_left(), color=GRID_COLOR, stroke_width=1, stroke_opacity=0.5))

        # --- ANIMATION SEQUENCE ---
        self.play(FadeIn(title, shift=UP*0.2))
        self.play(Create(input_grp), Create(router_grp), FadeIn(agents_grp, shift=LEFT*0.2))
        self.play(GrowArrow(arrow_in))
        
        # Show similarity across all agents
        self.play(Create(agent_paths), run_time=1.5)
        self.play(Indicate(agents_grp[1][2]), Indicate(agents_grp[3][2])) # Highlights 0.96 and 0.965
        
        # Show Ambiguity & LLM consultation
        tie_bracket = Brace(VGroup(agents_grp[1], agents_grp[3]), direction=RIGHT, color=PRIMARY_COLOR).scale(0.8)
        tie_text = Text("Semantic Tie", font_size=10, color=PRIMARY_COLOR).next_to(tie_bracket, RIGHT, buff=0.1)
        
        self.play(Create(tie_bracket), Write(tie_text))
        self.wait(0.5)
        self.play(FadeIn(llm_grp, shift=DOWN), Create(llm_comm))
        
        # Reasoning Pulse
        self.play(llm_box.animate.set_stroke(width=5), run_time=0.3)
        self.play(llm_box.animate.set_stroke(width=2), run_time=0.3)

        # Final Result
        final_path = Arrow(router_box.get_right(), agents_grp[3].get_left(), color=PRIMARY_COLOR, buff=0)
        self.play(
            FadeOut(tie_bracket), FadeOut(tie_text),
            Create(final_path),
            agents_grp[3][0].animate.set_color(PRIMARY_COLOR).set_stroke(width=3),
            agents_grp[3][1].animate.set_color(PRIMARY_COLOR)
        )
...
class HorizontalSentenceVectory(Scene):
    def construct(self):
        # Set background to white
        self.camera.background_color = WHITE
        ...
        # --- 1. TITLE SECTION ---
        # Requirement: BLACK and NOT bold
        title = Text("Sentence to Vector Mapping", color=BLACK_COLOR, font_size=32, weight=NORMAL)
        title.to_edge(UP, buff=0.7)

        # --- 2. MAIN COMPONENTS ---
        
        # A. The Input Sentence Box (The extraction result)
        sentence_box = RoundedRectangle(corner_radius=0.1, width=4.5, height=1.2, color=GRID_COLOR)
        sentence_text = Text(
            '"We need to reduce\ncloud costs."', 
            color=PRIMARY_COLOR, 
        ...
        # B. The Embedding Engine (Processing)
        engine_box = Rectangle(width=2.5, height=1.5, color=PRIMARY_COLOR, fill_color=PRIMARY_COLOR, fill_opacity=0.1)
        engine_label = Text("Embedding\nEngine", color=PRIMARY_COLOR, font_size=16, weight=BOLD)
        engine_label.move_to(engine_box.get_center())
        engine_group = VGroup(engine_box, engine_label)

        # C. The Horizontal Vector (Output)
        vector_math = MathTex(
            r"[ \ 0.54, \ -0.12, \ 0.89, \ 0.31, \ \dots \ ]",
            color=TEXT_COLOR
        ).scale(0.8)
        ...
        # --- 3. IMPROVED DOCUMENT ILLUSTRATION ---
        # Creating a realistic "Page" icon
        doc_width, doc_height = 1.6, 2.0
        doc_body = Rectangle(width=doc_width, height=doc_height, color=GRID_COLOR, fill_color=WHITE, fill_opacity=1)
        
        # Add "Text Lines" of varying lengths to look like a document
        line_widths = [0.8, 1.0, 0.7, 1.1, 0.9]
        text_lines = VGroup()
        for i, w in enumerate(line_widths):
            line_color = PRIMARY_COLOR if i == 1 else GRID_COLOR # Highlight the 2nd line
            line_thickness = 4 if i == 1 else 2
            l = Line(LEFT * (w/2), RIGHT * (w/2), color=line_color, stroke_width=line_thickness)
            text_lines.add(l)
        text_lines.arrange(DOWN, buff=0.25).move_to(doc_body.get_center())

        # Dog-ear corner (fold)
        corner_size = 0.3
        fold = Polygon(
            doc_body.get_corner(UR),
            doc_body.get_corner(UR) + DOWN * corner_size,
            doc_body.get_corner(UR) + LEFT * corner_size,
            color=GRID_COLOR,
            fill_color=GRID_COLOR,
            fill_opacity=1
        )
        
        doc_tag = Text("Source Document", color=TEXT_COLOR, font_size=12)
        document_illustration = VGroup(doc_body, text_lines, fold, doc_tag).arrange(DOWN, buff=0.2)

        # --- 4. LAYOUT ENGINE ---
        # Position the main chain
        main_flow = VGroup(input_group, engine_group, vector_group).arrange(RIGHT, buff=0.8)
        main_flow.move_to(UP * 0.5)

        # Positioning the Document strictly below the input group
        document_illustration.next_to(input_group, DOWN, buff=1.0)
        # Shift it slightly left to align with the center of the input box
        document_illustration.align_to(input_group, LEFT).shift(RIGHT * 0.5)

        # --- 5. CONNECTORS & LABELS ---
        arrow1 = Arrow(input_group.get_right(), engine_group.get_left(), color=GRID_COLOR, buff=0.1)
        arrow2 = Arrow(engine_group.get_right(), vector_group.get_left(), color=PRIMARY_COLOR, buff=0.1)
        
        input_tag = Text("Extracted Sentence", color=TEXT_COLOR, font_size=14).next_to(input_group, DOWN, buff=0.2)
        output_tag = Text("Vector Representation", color=TEXT_COLOR, font_size=14).next_to(vector_group, DOWN, buff=0.2)

        # --- ANIMATION SEQUENCE ---

        # 1. Reveal Title and the Document first
        self.play(Write(title))
        self.play(FadeIn(document_illustration, shift=UP))
        self.wait(0.5)

        # 2. Show the "Extraction" into the input box
        self.play(
            ReplacementTransform(text_lines[1].copy(), sentence_text),
            Create(sentence_box),
            FadeIn(input_tag)
        )
        self.wait(0.5)
...
class QuantumTextbook(ThreeDScene):
    def construct(self):
        # 1. Setup the Bloch Sphere (The Mental Model)
        axes = ThreeDAxes()
        sphere = Sphere(radius=2, resolution=(20, 20), fill_opacity=0.1)
        sphere.set_color(WHITE)
        
        # Labels for the poles (The Foundation)
        ket_0 = MathTex(r"|0\rangle").shift(2.5 * UP)
        ket_1 = MathTex(r"|1\rangle").shift(2.5 * DOWN)
        
        # 2. The Equation (The Dynamic Input)
        # Using ValueTrackers to allow "real-time" manipulation of alpha
        theta = ValueTracker(0)  # theta = 0 means purely |0> (alpha = 1)
        
        # 3. The State Vector (The Visualization)
        # alpha = cos(theta/2), beta = sin(theta/2)
        state_vector = always_redraw(lambda: Arrow3D(
            start=ORIGIN,
            end=[
        ...
        # 4. The Live Label
        # This updates as the user "drags" the alpha value
        equation_label = always_redraw(lambda: MathTex(
            rf"|\psi\rangle = {np.cos(theta.get_value()/2):.2f}|0\rangle + {np.sin(theta.get_value()/2):.2f}|1\rangle",
            color=YELLOW
        ).to_corner(UL))

        # --- ANIMATION SEQUENCE ---
        self.set_camera_orientation(phi=75 * DEGREES, theta=-45 * DEGREES)
        self.add(axes, sphere, ket_0, ket_1, equation_label)
        self.play(Create(state_vector))
        self.wait(1)

        # "Aha!" Moment: Change alpha to see the state move toward |1>
        self.play(
